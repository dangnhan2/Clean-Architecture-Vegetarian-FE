  ...
  const fetchFoodItems = async () => {
    ...
    if (res.isSuccess && Number(res.statusCode) === 200){
      if (res?.data){
        ...
        const onSaleItems = res.data.data?.filter(item => item.isOnSale) || [];
        setItems(onSaleItems);
        setTotal(onSaleItems.length);
      }
    }
  }
          ...
          <div className="flex flex-col lg:flex-row gap-4 md:gap-6 lg:gap-8">
            ...
            <div className="flex-1 lg:flex lg:flex-col">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-4 flex-wrap">
                <span className="text-gray-600 font-medium text-sm md:text-base">Sắp xếp:</span>
                  ...
                  {items && items.length > 0 ? (
                    items.map((item) => {
                      ...
                      return (
                        <Card
                          key={item.id}
                          className="overflow-hidden shadow-md hover:shadow-lg transition-shadow cursor-pointer border-2 border-red-100"
                          ...
                          <div className="relative aspect-[4/3] md:aspect-[16/9]">
                            <div className="absolute top-2 left-2 md:top-3 md:left-3 flex flex-col gap-1 md:gap-2 z-10">
                              {item.isOnSale && item.discountPercent && item.discountPercent > 0 && (
                                <Badge className="bg-red-500 text-white hover:bg-red-600 text-xs md:text-sm px-1.5 md:px-2 py-0.5 md:py-1 font-bold">
                    ...
                    <div className="col-span-1 md:col-span-3 p-8 md:p-12 text-center">
                      <Card className="p-6 md:p-8">
                        <p className="text-gray-600 text-base md:text-lg">
                          Không có món ăn nào đang giảm giá trong danh mục này.
                        </p>
                      </Card>
                    </div>
                  )}
